...
# model1 = load_model('C:/Users/EN308/Desktop/Kai/2class_MODEL/yun_normal/model/model_U_Net_yun_normal_CLAHE_0.0001_categorical_crossentropy_resnet50.hdf5', compile=False)
# val_img=getTestDate("C:/Users/EN308/Desktop/Kai/2class_MODEL/yun_normal/val/image_CLAHE")
...
#                     print(blackbone_functions[blackbone])2
...
#model1 = load_model('C:/Users/EN308/Desktop/Kai/2class_MODEL/yun_normal/model/model_unetplusplus_yun_normal_None_0.0001_total_loss_resnet50.hdf5', compile=False)
...
# X_train = np.concatenate([train_pred2_add, train_pred3_add, train_pred4_add, train_pred5_add], axis=3).astype(float)
# X_test = np.concatenate([pred2_add, pred3_add, pred4_add, pred5_add], axis=3).astype(float)

# X_train = np.concatenate([train_pred1_add, train_pred3_add, train_pred4_add], axis=3).astype(float)
# X_test = np.concatenate([pred1_add, pred3_add, pred4_add], axis=3).astype(float)
...
logistic_regr.fit(X_train, label_train)

#儲存模型
pickle.dump(logistic_regr, open('./ensemble_models/LR.pkl', 'wb'))


#載入模型
# logistic_regr = pickle.load(open('./ensemble_models/LR.pkl', 'rb'))

print("train...OK")

# 測試集的預測
yhat=logistic_regr.predict(X_test)

print("yhat_shape:", yhat.shape)
target_names = ['class 0', 'class 1', 'class 2']
#print(classification_report(label, yhat, target_names=target_names))
ensemble_prediction=yhat.reshape((33,512,512))
...
for SIZE_i in SIZE:
    ...
    for file in allFileList:
      if os.path.isdir(os.path.join(yourPath,file)):
        ...
        ax.set_title('Overlay results',fontsize=40)
        plt.axis('off')
