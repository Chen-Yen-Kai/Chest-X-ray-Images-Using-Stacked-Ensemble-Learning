...
#loss_functions =['categorical_crossentropy', focal_tversky, hybrid_loss]
#loss_functions =['categorical_crossentropy', focal_tversky, total_loss, hybrid_loss]
loss_functions = [total_loss]

#loss_functions_str =['categorical_crossentropy', focal_tversky, hybrid_loss]
# loss_functions_str =['categorical_crossentropy', 'focal_tversky', 'total_loss', 'hybrid_loss']
...
def pandas_classification_report(y_true, y_pred):
    ...
    metrics_sum_index = ['precision', 'recall', 'f1-scoㄅre', 'support']
            ...
            for blackbone in range(len(blackbone_functions)):
                ...
                for loss in range(len(loss_functions_str)):
                    ...
                    for rate in range(len(rate_functions)):
                        ...
                        es = EarlyStopping(monitor='val_iou_score', patience=10, mode='max', restore_best_weights=True)
                        count = 0
                        while Mean_IoU<0.65:
                            count += 1
                            ...
                            if blackbone_functions[blackbone] == "xception" :
                                if net_str[net] == "Deeplabv3plus" :
