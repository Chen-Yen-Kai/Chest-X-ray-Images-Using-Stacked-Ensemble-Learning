# modified code from https://github.com/MrGiovanni/UNetPlusPlus/blob/master/keras/segmentation_models/unet/builder.py
...
from ..blocks import conv_block, attention_block
...
def build_unet(use_backbone, backbone, classes, skip_connection_layers,
    ...
    #x = tfa.layers.SpatialPyramidPooling2D([1, 3, 5],data_format='channels_last')(x)
    for i in range(n_upsample_blocks):
        ...
        if i < len(skip_layers_list):
            ...
            if attention:
                skip_connection = attention_block(
                    decoder_filters[i], skip_connection,
                    n_upsample_blocks-i-1, i+1, upsample_rate=upsample_rate) (x)
